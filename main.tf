...
locals {
  aws_key = "Medha-SWEN-614" # SSH key pair name for EC2 instance access
}
...
resource "aws_security_group" "http_securitygroup" {
  name        = "allow_http"
...
resource "aws_instance" "my_server" {
  ami           = data.aws_ami.amazonlinux.id # Use the AMI ID from the data so>
  instance_type = var.instance_type           # Use the instance type from vari>
  key_name      = local.aws_key               # Specify the SSH key pair name
  # Attach the security group
  security_groups = [aws_security_group.http_securitygroup.name]
